...
SkipList<Key, Comparator>::SkipList(Comparator cmp, Arena* arena)
    : UseBloomFilter(false),
      ...
      compare_(cmp),
      arena_(arena),
      head_(NewNode(0 /* any key will do */, kMaxHeight, 0 /*add */)),
      max_height_(1),
      rnd_(0xdeadbeef) {
...
//---
template <typename Key, class Comparator>
void MergeSkipLists(SkipList<Key, Comparator>& PMtableA, const SkipList<Key, Comparator>& PMtableB, SkipList<Key, Comparator>& newPMtable) {

    std::vector<Key> Array;  // To store duplicated keys

    typename SkipList<Key, Comparator>::Iterator x(&PMtableA);
    typename SkipList<Key, Comparator>::Iterator y;

    y.SeekToFirst(&PMtableB);
    x.SeekToFirst();
    ...
    while (x.Valid()) {
        ...
        while (y.Valid()) {
            if (areNodesDuplicates(x.node_, y.node_)) {
                // If nodes x and y are duplicates
                Array.push_back(x.key());
            } else {
                // Directly update to the New PMtable
                newPMtable.Insert(y.key());
            }
            y.Next();
        }
        
        // Don't forget to add keys from PMtableA to newPMtable
        newPMtable.Insert(x.key());

        x.Next();
    }

    // Update valid keys from Array to the New PMtable
    for (const Key& key : Array) {
        newPMtable.Insert(key);
    }
}
...
SkipList<Key, Comparator>::SkipList(Comparator cmp)
    : UseBloomFilter(false),
      ...
      compare_(cmp),
      arena_(nullptr),
      head_(LastTableNewNode(0 /* any key will do */, kLastHeight, 0 /*add*/)),
      max_height_(1),
      rnd_(0xdeadbeef) {
